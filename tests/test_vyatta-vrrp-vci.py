...
class TestVyattaVrrpVci(TestCase):
    ...
    def setup_method(self, method):
        ...
        self.vif_two_type_list_sanitized = [self.vif_dataplane_list[0],
                                            self.vif_bonding_list[0]]
    ...
    def test_sanitize_vrrp_config_one_configured_one_not_configured(self):
        ...
        test_conf[self.INTERFACE_YANG_NAME][self.DATAPLANE_YANG_NAME]\
            .append(self.second_intf)
        assert test_conf != expected
        result = self.test_conf._sanitize_vrrp_config(test_conf)
        assert result == expected
    ...
    def test_sanitize_vrrp_config_two_intf_type(self):
        ...
        expected["vyatta-interfaces-v1:interfaces"]["vyatta-bonding-v1:bonding"]\
            = self.bonding_list
        test_conf = copy.deepcopy(self.simple_conf)
        test_conf[self.INTERFACE_YANG_NAME][self.DATAPLANE_YANG_NAME]\
            .append(self.second_intf)
        test_conf[self.INTERFACE_YANG_NAME][self.BONDING_YANG_NAME] =\
            self.bonding_list
        assert test_conf != expected
        result = self.test_conf._sanitize_vrrp_config(test_conf)
        assert result == expected
    ...
    def test_sanitize_vrrp_config_remove_empty_vif(self):
        expected = copy.deepcopy(self.simple_conf)
        test_conf = copy.deepcopy(self.simple_conf)
        test_conf[self.INTERFACE_YANG_NAME][self.DATAPLANE_YANG_NAME][0]["vif"]\
            = self.vif_dataplane_list
        result = self.test_conf._sanitize_vrrp_config(test_conf)
    ...
    def test_sanitize_vrrp_config_move_dataplane_vif(self):
        expected = copy.deepcopy(self.simple_conf)
        expected["vyatta-interfaces-v1:interfaces"]["vif"] =\
            self.vif_dataplane_list_sanitized
        test_conf = copy.deepcopy(self.simple_conf)
        test_conf[self.INTERFACE_YANG_NAME][self.DATAPLANE_YANG_NAME][0]["vif"]\
            = self.vif_dataplane_list
        test_conf[self.INTERFACE_YANG_NAME][self.DATAPLANE_YANG_NAME][0]["vif"]\
            [0]["vyatta-vrrp-v1:vrrp"] = self.vif_dataplane_vrrp_group
        result = self.test_conf._sanitize_vrrp_config(test_conf)
        assert expected == result
    ...
    def test_sanitize_vrrp_config_move_bonding_vif(self):
        ...
        test_conf[self.INTERFACE_YANG_NAME][self.BONDING_YANG_NAME] =\
            self.bonding_list
        test_conf[self.INTERFACE_YANG_NAME][self.BONDING_YANG_NAME][0]["vif"] =\
            self.vif_bonding_list
        test_conf[self.INTERFACE_YANG_NAME][self.BONDING_YANG_NAME][0]\
            ["vif"][0]["vyatta-vrrp-v1:vrrp"] = self.vif_bonding_vrrp_group
        result = self.test_conf._sanitize_vrrp_config(test_conf)
        assert expected == result
    ...
    def test_sanitize_vrrp_config_move_two_intf_types_vif(self):
        ...
        expected["vyatta-interfaces-v1:interfaces"]["vyatta-bonding-v1:bonding"]\
            = self.bonding_list
        expected["vyatta-interfaces-v1:interfaces"]["vif"] =\
            self.vif_two_type_list_sanitized
        test_conf = copy.deepcopy(self.simple_conf)
        test_conf[self.INTERFACE_YANG_NAME][self.DATAPLANE_YANG_NAME][0]["vif"]\
            = self.vif_dataplane_list
        test_conf[self.INTERFACE_YANG_NAME][self.DATAPLANE_YANG_NAME][0]["vif"]\
        ...
        test_conf[self.INTERFACE_YANG_NAME][self.BONDING_YANG_NAME][0]["vif"] =\
            self.vif_bonding_list
        test_conf[self.INTERFACE_YANG_NAME][self.BONDING_YANG_NAME][0]["vif"]\
            [0]["vyatta-vrrp-v1:vrrp"] = self.vif_bonding_vrrp_group
        result = self.test_conf._sanitize_vrrp_config(test_conf)
        assert expected == result
